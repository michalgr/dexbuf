...
class TestDexFile(unittest.TestCase):
    ...
    def test_dex_with_scoped_mmap(self) -> None:
        """Verify DexFile initialized via scoped_mmap."""
        with tempfile.NamedTemporaryFile(suffix=".dex", delete=True) as tmp:
            ...
            with scoped_mmap(tmp.name) as mm:
                ...
                self.assertTrue(dex.verify_signature())
                self.assertEqual(dex.get_string(Idx[StringIdItem](0)), "I")

    def test_dex_with_open_mmap(self) -> None:
        """Verify DexFile initialized via open_mmap."""
        with tempfile.NamedTemporaryFile(suffix=".dex", delete=True) as tmp:
            tmp.write(self.dex_bytes)
            tmp.flush()
    ...
    def test_header_verification(self) -> None:
        ...
        # 6. DexFile verify parameter pass-through
        with self.assertRaises(ValueError):
            DexFile(corrupt_magic_buf, verify=True)

        opened_unverified = DexFile(corrupt_magic_buf, verify=False)
