...
class DbgStartLocal:
    ...
    def to_bytes(self) -> bytes:
        ...
        return (
            ...
            + encode_uleb128p1(-1 if self.name_idx is None else self.name_idx)
            + encode_uleb128p1(-1 if self.type_idx is None else self.type_idx)
        )
...
class DbgStartLocalExtended:
    ...
    def to_bytes(self) -> bytes:
        ...
        return (
            ...
            + encode_uleb128p1(-1 if self.name_idx is None else self.name_idx)
            + encode_uleb128p1(-1 if self.type_idx is None else self.type_idx)
            + encode_uleb128p1(-1 if self.sig_idx is None else self.sig_idx)
        )
...
class DbgSetFile:
    ...
    def to_bytes(self) -> bytes:
        ...
        return b"\x09" + encode_uleb128p1(-1 if self.name_idx is None else self.name_idx)
